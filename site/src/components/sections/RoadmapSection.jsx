...
function RoadmapColumn({ label, title, items, toneClass, theme }) {
  return (
    <div className={`rounded-[1.7rem] border p-6 ${toneClass}`}>
      <div className={`text-xs font-semibold uppercase tracking-[0.24em] ${theme.accentText}`}>
        {label}
      </div>
    ...
    <section className="mx-auto max-w-7xl px-6 py-20 lg:px-10">
      <div className={`rounded-[2rem] border p-8 ${theme.card}`}>
        ...
        <div className="mt-10 grid gap-5 xl:grid-cols-3">
          ...
          <RoadmapColumn
            {...t.roadmap.columns.later}
            toneClass={theme.roadmapLater}
            theme={theme}
          />
        </div>
        <div className={`mt-8 rounded-[1.5rem] border p-5 ${theme.recommendation}`}>
          <div className={`text-xs font-semibold uppercase tracking-[0.2em] ${theme.panelMuted}`}>
            {t.roadmap.recommendationLabel}
          </div>
          <p className={`mt-3 max-w-4xl text-sm leading-7 ${theme.body}`}>
            {t.roadmap.recommendation}
          </p>
        </div>
      </div>
    </section>
