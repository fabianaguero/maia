    ...
    <section id="mvp" className="mx-auto max-w-7xl px-6 py-20 lg:px-10">
      <div className="grid gap-10 lg:grid-cols-[1.05fr_0.95fr]">
        <div className={`rounded-[2rem] border p-8 ${theme.cardAccent}`}>
          <div className={`mb-4 inline-flex items-center gap-2 rounded-full border px-4 py-2 text-xs font-medium uppercase tracking-[0.22em] ${theme.soft} ${theme.body}`}>
          ...
          <div className="mt-8 grid gap-3 sm:grid-cols-2">
            {t.mvp.items.map((item) => (
              <div key={item} className={`rounded-2xl border px-4 py-3 text-sm ${theme.soft} ${theme.body}`}>{item}</div>
            ))}
          </div>
        </div>
