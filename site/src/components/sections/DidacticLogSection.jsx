  ...
  return (
    <section className="mx-auto max-w-7xl px-6 py-20 lg:px-10">
      <div className="grid gap-10 lg:grid-cols-[0.95fr_1.05fr] lg:items-start">
        <SectionTitle
          eyebrow={t.didactic.eyebrow}
          title={t.didactic.title}
          text={t.didactic.text}
          theme={theme}
        />
        ...
        <div className="grid gap-5 xl:grid-cols-[0.95fr_1.05fr]">
            ...
            <div className="mb-4 flex items-center justify-between">
              <div>
                <div className={`text-sm font-semibold ${theme.title}`}>
                  {t.didactic.inputTitle}
                </div>
                <div className={`text-xs uppercase tracking-[0.18em] ${theme.panelMuted}`}>
                  {t.didactic.inputSubtitle}
                </div>
              </div>
              <div
                className={`rounded-full border px-3 py-1 text-[11px] uppercase tracking-[0.18em] ${theme.soft} ${theme.panelMuted}`}
              >
                payments-api
              </div>
            </div>
              ...
              {t.didactic.logExample.map((entry, index) => {
                ...
                return (
                  <div
                    ...
                    className={`rounded-2xl border px-3 py-2 ${theme.soft}`}
                  >
                    <div className="flex flex-wrap items-center gap-2">
                      <span className={theme.panelMuted}>{entry.time}</span>
                      <span
                        className={`rounded-full border px-2 py-0.5 text-[10px] font-semibold uppercase tracking-[0.16em] ${levelTone}`}
                      >
                        {entry.level}
                      </span>
          ...
          <div className={`rounded-[1.8rem] border p-5 ${theme.card}`}>
            <div className="mb-4 flex items-center justify-between">
              <div>
                <div className={`text-sm font-semibold ${theme.title}`}>
                  {t.didactic.outputTitle}
                </div>
                <div className={`text-xs uppercase tracking-[0.18em] ${theme.panelMuted}`}>
                  {t.didactic.outputSubtitle}
                </div>
              </div>
            ...
            <div className={`rounded-[1.4rem] border p-4 ${theme.soft}`}>
              <div
                className={`mb-4 flex items-center justify-between text-xs uppercase tracking-[0.16em] ${theme.panelMuted}`}
              >
                ...
                {t.didactic.sonicMapping.map((row) => (
                  <div key={row.label}>
                    <div className="mb-2 flex items-center justify-between gap-4">
                      ...
                      <div
                        className={`text-[11px] uppercase tracking-[0.16em] ${theme.panelMuted}`}
                      >
                        {t.didactic.mappedCue}
                      </div>
                    </div>
                    <div className={`h-3 rounded-full ${isDark ? "bg-white/5" : "bg-slate-200"}`}>
                      <div
                        className="h-3 rounded-full bg-gradient-to-r from-cyan-300 via-white to-violet-300"
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              <div className={`rounded-2xl border px-4 py-3 ${theme.soft}`}>
                <div className={`text-xs uppercase tracking-[0.16em] ${theme.panelMuted}`}>
                  {t.didactic.patternLogicTitle}
                </div>
                <div className={`mt-2 text-sm ${theme.body}`}>{t.didactic.patternLogicText}</div>
              </div>
              <div className={`rounded-2xl border px-4 py-3 ${theme.soft}`}>
                <div className={`text-xs uppercase tracking-[0.16em] ${theme.panelMuted}`}>
                  {t.didactic.operatorValueTitle}
                </div>
