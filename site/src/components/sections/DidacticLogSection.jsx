  ...
  return (
    <section className="mx-auto max-w-7xl px-6 py-20 lg:px-10">
      <div className="grid gap-10 lg:grid-cols-[0.95fr_1.05fr] lg:items-start">
        <SectionTitle eyebrow={t.didactic.eyebrow} title={t.didactic.title} text={t.didactic.text} classes={{ eyebrow: theme.eyebrow, title: theme.title, body: theme.body }} />
        <div className="grid gap-5 xl:grid-cols-[0.95fr_1.05fr]">
          ...
          <div className={`rounded-[1.8rem] border p-5 ${theme.card}`}>
            <div className="mb-4 flex items-center justify-between">
              ...
              <div className="rounded-full border border-violet-400/30 bg-violet-400/10 px-3 py-1 text-[11px] uppercase tracking-[0.18em] text-violet-200">{t.didactic.outputBadge}</div>
            </div>
              ...
              <div className="space-y-4">
                {t.didactic.sonicMapping.map((row) => (
                  <div key={row.label}>
                    ...
                    <div className={`h-3 rounded-full ${isDark ? "bg-white/5" : "bg-slate-200"}`}><div className="h-3 rounded-full bg-gradient-to-r from-cyan-300 via-white to-violet-300" style={{ width: row.width }} /></div>
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              <div className={`rounded-2xl border px-4 py-3 ${theme.soft}`}>
                ...
                <div className={`mt-2 text-sm ${theme.title}`}>{t.didactic.patternLogicText}</div>
              </div>
              <div className={`rounded-2xl border px-4 py-3 ${theme.soft}`}>
                <div className={`text-xs uppercase tracking-[0.16em] ${theme.panelMuted}`}>{t.didactic.operatorValueTitle}</div>
                <div className={`mt-2 text-sm ${theme.title}`}>{t.didactic.operatorValueText}</div>
              </div>
            </div>
          </div>
        </div>
