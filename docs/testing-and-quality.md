...
- `desktop/src/features/monitor/monitorProviderOrchestrationRuntime.ts` now centralizes the pure input-shaping layer for provider update emission, polling, replay timing, guide-track cursor sync, and manual audio resume, keeping `useMonitorProviderRuntimeOrchestration.ts` closer to React lifecycle wiring while direct runtime tests protect the orchestration contract
