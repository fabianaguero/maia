...
export function useLiveLogMonitorPlayback(input: LiveLogMonitorPlaybackInput) {
  const playWithCurrentEngine = useEffectEvent((cues: RoutedLiveCue[], liveBpm?: number | null) => {
    ...
    const preferGuideTrackMutation =
      input.playableBaseTracks.length > 0 && input.backgroundDeckRef.current !== null;
      ...
      for (const scheduledEvent of graphSchedulePlan.events) {
        ...
        const sampleBuffer =
          input.sampleStatus === "ready" && voice.cue.samplePath && voice.track === "foundation"
            ? (input.sampleBuffersRef.current.get(voice.cue.samplePath) ?? null)
            : null;
    ...
    } else if (graphSchedulePlan.shouldResumeSuspendedContext && context) {
      void context.resume();
    }

    input.setEmittedVoiceCount((c) => c + playbackPlan.voicedCues.length);
  });
