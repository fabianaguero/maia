...
import { BOUNCE_WINDOW_S, renderBounceWav } from "./wavRenderer";

export interface LiveLogMonitorSessionStartActionInput {
...
export interface LiveLogMonitorSessionStopActionInput {
  monitor: {
...
export interface LiveLogMonitorSessionBounceActionInput {
  repository: RepositoryAnalysis;
  bounceCuesRef: MutableRefObject<RoutedLiveCue[][]>;
  masterVolume: number;
}

export function buildLiveLogMonitorStartWarningUpdater(warning: string) {
  return (current: string[]) => [warning, ...current];
}

export async function startLiveLogMonitorSessionAction(
  input: LiveLogMonitorSessionStartActionInput,
): Promise<void> {
  ...
  try {
    ...
    if (startWarning) {
      input.setRecentWarnings(buildLiveLogMonitorStartWarningUpdater(startWarning));
    }
...
export function stopLiveLogMonitorSessionAction(input: LiveLogMonitorSessionStopActionInput): void {
...
export function bounceLiveLogMonitorSessionAction(
  input: LiveLogMonitorSessionBounceActionInput,
): void {
