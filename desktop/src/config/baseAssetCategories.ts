...
import type { BaseAssetCategoryCatalog, BaseAssetCategoryOption } from "../types/baseAsset";
...
function normalizeCatalog(input: Partial<BaseAssetCategoryCatalog>): BaseAssetCategoryCatalog {
  const baseAssetCategories = Array.isArray(input.baseAssetCategories)
    ? input.baseAssetCategories
        .map((category) => normalizeCategory(category))
        .filter((category): category is BaseAssetCategoryOption => category !== null)
    : [];
...
export function resolveBaseAssetCategory(
  ...
  return (
    baseAssetCategoryCatalog.baseAssetCategories.find((category) => category.id === id) ?? null
  );
}

export function fallbackBaseAssetCategoryLabel(id: string | null | undefined): string {
