...
import { buildSessionScreenControllerMonitorSnapshot } from "../../../src/features/session/sessionScreenControllerHookRuntime";
import { useSessionScreenControllerSlices } from "../../../src/features/session/useSessionScreenControllerSlices";

describe("useSessionScreenControllerSlices", () => {
  it("composes actions, derived state, effects and booth state behind one seam", () => {
    const input = {
      tracks: [{ id: "track-1" }],
      playlists: [],
    ...
    const localState = {
      ...
      selectedTemplateId: "steady-house",
    ...
    expect(result.current.actions).toBe(mocks.actions);
    expect(result.current.derivedState).toBe(mocks.derivedState);
