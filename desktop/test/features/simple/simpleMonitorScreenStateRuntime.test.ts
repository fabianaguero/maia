...
import {
  ...
  getSimpleMonitorTrackTitle,
} from "../../../src/features/simple/simpleMonitorScreenStateRuntime";
...
describe("simpleMonitorScreenStateRuntime", () => {
  it("coerces collections and exposes track title helper", () => {
    ...
    expect(collections.safePastSessions).toHaveLength(1);
    expect(collections.safeRepositories).toHaveLength(1);
    expect(collections.safeTracks).toHaveLength(1);
    expect(getSimpleMonitorTrackTitle(track)).toBe("Deck Track");
  });

  it("builds final hook-state args from launch and deck state slices", () => {
    const track = createTrack();
    const activeSession = createActiveSession();
    const hookArgs = buildSimpleMonitorScreenHookStateArgs({
    ...
    expect(hookArgs.uptimeLabel).toBe("15s");
    expect(hookArgs.deckRemainingSeconds).toBe(208);
    expect(hookArgs.getTrackTitle(track)).toBe("Deck Track");
    expect(hookArgs.sessions[0]?.id).toBe("session-1");
  });
});
