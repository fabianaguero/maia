...
import { useSimpleMonitorDeckControllerSlices } from "../../../src/features/simple/useSimpleMonitorDeckControllerSlices";
...
describe("useSimpleMonitorDeckControllerSlices", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("builds runtime, model, live, and presentation slices for the deck controller", () => {
    ...
    expect(result.current.runtimeInput.skin).toBe("copper");
    expect(result.current.controllerModel.activeTrack?.id).toBe("track-1");
    expect(result.current.deckBpm).toBe(132);
    expect(result.current.liveState.previewTrackId).toBe("track-1");
    expect(result.current.presentationState.overviewWindowLeftPercent).toBe(0.1);
